...
import { getYouTubeId } from './VideoCard.jsx';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 p-4 sm:p-6">
      <div className="relative w-full max-w-3xl rounded-[28px] border border-white/10 bg-slate-900/95 shadow-[0_30px_80px_rgba(0,0,0,0.55)] max-h-[calc(100vh-3rem)] overflow-hidden">
        <button onClick={onClose} className="absolute right-4 top-4 inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-slate-950/80 text-slate-100 transition duration-300 hover:bg-slate-800/90 hover:-translate-y-0.5">
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="p-6 sm:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button onClick={onClose} className="rounded-full border border-white/10 bg-slate-950/80 px-4 py-2 text-sm font-semibold text-slate-100 transition duration-300 hover:-translate-y-0.5 hover:border-cyan-400/50 hover:bg-slate-800/90">
              Close
            </button>
          </div>
